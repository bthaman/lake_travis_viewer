...
def import_gage_data(site_num, full_level=0):
    ...
    if action == 'get_data':
        ...
        if download_file:
            ...
            sgrpi = np.array(sgrp.keys().get_values(), dtype='datetime64[D]')
...
def subplots(x, y, header, count, subplot_num, latest_vals=None, full=None, show_axis_labels=True,
    ...
    plt.annotate(str_annotation_current, xy=(x[-1], current_val), xytext=(0.65, 0.5), textcoords='axes fraction',
    ...
    plt.fill_between(x, y, y2=plt.ylim()[0], facecolor='', alpha=0.0)
